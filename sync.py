...
def make_semaphore(device, debug):
    ...
    try:
        ...
        if debug: print("Failed to create semaphore")
        return None


def make_fence(device, debug):

    fenceInfo = VkFenceCreateInfo(flags=VK_FENCE_CREATE_SIGNALED_BIT) # flag means the fence starts opened

    try:
        return vkCreateFence(device, fenceInfo, None)
    except:
        if debug: print("Failed to create fence")
        return None